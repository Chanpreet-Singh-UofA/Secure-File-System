...
def login(command):
    ...
    global currentUser
    global at_end_encrypt_these_paths
    global cursor
    global sfsDirectory
    global directory
    global sfs
    ...
    while(True):
        ...
        try:
            ...
            if (command.find("mkgroup")!=-1):      # mkgroup groupA
                ...
                shift = 3
                encryptedFileName = cipher_encrypt(command[1], shift)
                cursor.execute("UPDATE sfsFiles SET fileName=\""+command[2]+"\", fileEncryptedName=\""+encryptedFileName+"\", fileDirectory=\""+new_name+"\" WHERE fileDirectory=\""+old_name+"\"")
                connection.commit()
                os.rename(old_name, new_name)  
                at_end_encrypt_these_paths.remove(old_name)
                at_end_encrypt_these_paths.append(new_name)
                print("File renamed.")

            elif(command.find("rm")!=-1): #rm filename
                command= command.split(" ")
                filename = command[1]
                fileDir = os.path.abspath(filename)
                cursor.execute("DELETE FROM sfsFiles WHERE fileDirectory = ?", (fileDir,))
                connection.commit()
                at_end_encrypt_these_paths.remove(fileDir)
                os.remove(command[1])
                print("File deleted.")
                ...
                print("* To rename a file - rename <old name> <new name>")
                print("* To delete a file - rm <file name>")
